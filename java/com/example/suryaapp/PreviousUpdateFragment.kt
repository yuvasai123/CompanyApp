...
class PreviousUpdateFragment : Fragment() {

    private val args by navArgs<PreviousUpdateFragmentArgs>()
    private lateinit var previousViewModel: PreviousViewModel
    ...
    override fun onCreateView(
        ...
        val itemView = inflater.inflate(R.layout.fragment_previous_update, container, false)

        previousViewModel = ViewModelProvider(this).get(PreviousViewModel::class.java)
        ...
        // Set values
        customerName.setText(args.PreviousOrder.customerName)
        val dateFormatter = SimpleDateFormat("dd-MM-yyyy", Locale.getDefault())
        val dateString = dateFormatter.format(args.PreviousOrder.date)
        date.text = dateString
        tmq.setText(args.PreviousOrder.tmq)
        chq.setText(args.PreviousOrder.chq)
        vgq.setText(args.PreviousOrder.vgq)
        soq.setText(args.PreviousOrder.soq)
        tm5q.setText(args.PreviousOrder.tm5q)
        ch5q.setText(args.PreviousOrder.ch5q)
        so5q.setText(args.PreviousOrder.so5q)
        amount.setText(args.PreviousOrder.amount)
    ...
    private fun updateItem() {
        ...
        val balance = amount.text.toString()




        val updatedOrder = Previous(
            args.PreviousOrder.id,
            updatedCustomerName,
            updatedDate,
            updatedTmq,
            updatedChq,
        ...
        if (updatedCustomerName.isEmpty()) {
            ...
            previousViewModel.updatePending(updatedOrder)
            findNavController().navigate(R.id.action_previousUpdateFragment_to_previousListFragment)
